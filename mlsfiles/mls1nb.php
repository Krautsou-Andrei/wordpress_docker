  ...
  function start()
  {
    ...
    if ($this->realty_type == 'stats') {
      ...
      if (isset($_POST['UPD'])) {
        ...
        $whereR = '';
        if ($whereS == '') {
          if (isset($_GET['raions'])) foreach (explode(',', $_GET['raions'][0]) as $rn) if (intval($rn)) $whereS .= intval($rn) . ',';
          $whereS = substr($whereS, 0, strlen($whereS) - 1);
          if ($whereS != '') $this->gets .= '&raions[]=' . $whereS;
        } else {
          if ($whereS != '') $this->gets .= '&raions[]=' . $whereS;
        ...
        /*
         if ($whereH==''){
           if(isset($_GET['htype']))foreach(explode(',',$_GET['htype'][0]) as $rn) if(intval($rn)) $whereH.=intval($rn).',';
           $whereH=substr($whereH,0,strlen($whereH)-1);
           if ($whereH!='')$this->gets .='&htype[]='.$whereH;
         }
         else {
           if ($whereH!='')$this->gets .='&htype[]='.$whereH;
...
*/
      ...
      if ($this->filter == 1) {
        $sql_price = mysqli_query($this->dbcnx, 'select MIN(PRICE),MAX(PRICE) from APARTS where ' . (isset($this->stype) && $this->stype == 1 ? 'VARIANT=2' : 'VARIANT<>2') . ' and NEWBUILD=1 and WHAT=0 and COMPID=' . $this->agency);
        $dataPrice = mysqli_fetch_row($sql_price);
        $sql_area = mysqli_query($this->dbcnx, 'select MIN(AAREA),MAX(AAREA) from APARTS where ' . (isset($this->stype) && $this->stype == 1 ? 'VARIANT=2' : 'VARIANT<>2') . ' and NEWBUILD=1 and WHAT=0 and COMPID=' . $this->agency);
...
<div class="row realty_filter search_filter"><div class="col-xs-12 realty_filter__wrapper">
 	 ...
 	 <div class="op10 col-lg-2 col-md-4 col-sm-6 col-xs-12">
					...
					<select class="form-control" name="razdel" onchange="javascript:location=this.value;">
                        ...
                        <option value="' . $this->pagelink1nb . '" selected>Новостройки</option>
...
<div class="row realty_filter search_filter"><div class="col-xs-12">
   ...
   <div class="mt1 col-xs-12">
    ...
    <div class="mt2 col-xs-12">
       <strong>Площадь, м²</strong><span id="mlsAreaM"> от ' . (isset($this->area_ot) && $this->area_ot > 0 ? $this->area_ot : $dataAreaMIN) . ' до ' . (isset($this->area_do) && $this->area_do > 0 ? $this->area_do : $dataAreaMAX) . '</span><br />
	    ...
	    <div class="rooms_f mt2 col-xs-12">
	       <b>Комнат</b></br>
  ...
  function createStats($stats, $rc)
  {
    ...
    $total_posts = $found_posts + $rc;

    echo '<div class="row">
            <div class="col-xs-12">
            <div class="catalog__subtitle">' . num_word($total_posts, array('Найдена', 'Найдено', 'Найдено'), false) . ' ' . num_word($total_posts, array('квартира', 'квартиры', 'квартир')) . '</div>
    ...
    if ($query->have_posts()) {
      ...
      $postIds = $cookieValue ? array_map('intval', $postIds) : [];

      $option_category_card = array(
        'imgpath' => $this->imgpath,
        'scriptpath' => $this->scriptpath,
        'avatarpath' => $this->avatarpath,
        'arrayCategories' => $postIds,
        'isPost' => true
      );
      ...
      while ($query->have_posts()) {
        ...
        $is_studio_apartment = carbon_get_post_meta(get_the_ID(), 'product_type_aparts');
        $product_rooms = carbon_get_post_meta(get_the_ID(), 'product-rooms');
        $product_area = carbon_get_post_meta(get_the_ID(), 'product-area');
        $product_stage = carbon_get_post_meta(get_the_ID(), 'product-stage');

        $post_permalink = get_permalink(get_the_ID());
        $product_update_date = get_the_date( 'Y-m-d H:i:s', get_the_ID() );  
        $image_url = get_template_directory_uri() . '/assets/images/no_image.svg';
        $image_urls = array();
        $aget_photo = "";

        if(!empty($product_agent_photo)){
          $aget_photo =  wp_get_attachment_image_src($product_agent_photo, 'full')[0];
        }
        ...
        $author = array(
          ...
          'STAGE' => $product_stage,
          'CITY' => $product_city,
          'RAION' => $product_sub_locality,
          'STREET' => $product_street,
          'PRICE' => $product_price,
          'SQMPRICE' => $product_price_meter,
          'MISC' => $product_description,
          'PHOTO' =>  $aget_photo,
          "PHONE" => $product_agent_phone,
          'DATECOR' => $product_update_date,
          // 'GK'=>,
          'IMAGE' => $image_url,
          'PRODUCT_LABEL' => $product_label,
          'IS_STUDIO_APARTMENT' => $is_studio_apartment,
  ...
  function showObject($fields)
  {
    ...
    <h3>Кто-то его купил, или снял с продажи, но если вы посмотрите на сайте вы возможно найдете подходящий вариант.</h3>';
      ...
      $pTtl1 = $town . ", " . ($cnt["RPLANID"] != 622 ? $cnt["Комнат"] . '-комнатная квартира' : 'студия') . ", " . $cnt["Тип дома"] . ", на " . $cnt["Улица"] . ", " . $cnt["Этаж"] . ", " . $cnt["Материал стен"] . ",площадь: " . $cnt["AAREA"] . " кв.м, цена: " . number_format($cnt["Цена общая"] / 1000, 3, ' ', ' ') . " рублей.";
...
<div class="main-second-product__page">
    <section class="single-page">
      ...
      if ($pics != '') {
        ...
        echo '<img class="product-image-wrapper__preview" data-type="popup-gallery" src="' . $imgpath1 . '" data-post-id="' . $cnt['CARDNUM'] . '">';
        echo '</div>
			</div>
...
<div class="product__gallery">
      ...
      if ($cnt["YBUILD"] != '') echo '<div class="more-column__row"><span>Сдача</span><span>' . $cnt["YBUILD"] . '</span></div>';
